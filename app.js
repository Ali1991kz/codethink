...
function normalize_(value = '') { return String(value).trim().toLocaleLowerCase('kk-KZ'); }
...
function renderProgress() {
  ...
  const assignments = state.workspace.assignments || [], taskDone = assignments.filter(a => ['submitted','graded'].includes(a.attempt?.status)).length;
  const assignmentItems = assignments.map(a => `<li><span>${escapeHtml(a.title)}</span><b>${assignmentStatus(a.attempt?.status || '')}</b></li>`).join('') || '<li><span>Әзірге тапсырма берілмеген</span><b>—</b></li>';
  const achievements = [
    { icon:'🚀', title:'Алғашқы қадам', text:'Платформаға кірді', earned:true },
    { icon:'◎', title:'Диагностика', text:'Бастапқы деңгей анықталды', earned:baseline?.status === 'graded' },
    { icon:'⌘', title:'5 кезең шебері', text:'Барлық кезеңді аяқтады', earned:methodDone === 5 },
    { icon:'🐞', title:'Қате іздеуші', text:'Тексеру кезеңін орындады', earned:methodDone >= 4 || taskDone >= 1 },
    { icon:'🐍', title:'Python зерттеушісі', text:'Алғашқы тапсырманы тапсырды', earned:taskDone >= 1 },
    { icon:'🏅', title:'10 тапсырма', text:'10 жұмысты аяқтады', earned:taskDone >= 10 },
    { icon:'📈', title:'Өсім', text:'Нәтижесін жақсартты', earned:baseline?.status === 'graded' && finalAttempt?.status === 'graded' && Number(finalAttempt.totalScore) > Number(baseline.totalScore) },
    { icon:'⭐', title:'Тұрақты нәтиже', text:'5 тапсырманы аяқтады', earned:taskDone >= 5 }
  ];
  const badges = achievements.filter(x=>x.earned).map(x=>x.title);
  const nextAssignment = assignments.find(a => !['submitted','graded'].includes(a.attempt?.status));
  const nextAction = nextAssignment ? `«${nextAssignment.title}» тапсырмасын жалғастырыңыз.` : recommendation;
  $('#progressContent').innerHTML = `<div class="profile-banner"><div class="profile-identity"><div class="profile-avatar">${escapeHtml(state.user.name.charAt(0).toUpperCase())}</div><div><small>ОҚУШЫНЫҢ ЖЕКЕ КАБИНЕТІ</small><h2>${escapeHtml(state.user.name)}</h2><p>${escapeHtml(state.user.className || state.user.grade + '-сынып')} · ${escapeHtml(state.user.login)} · соңғы кіру: ${formatDate(state.user.lastLoginAt)}</p></div></div><div class="badge-row">${badges.slice(0,4).map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div></div><div class="progress-hero"><article class="metric-card"><small>Соңғы диагностика</small><strong>${diagnosticStatus}</strong><p>${attempt?.teacherComment ? escapeHtml(attempt.teacherComment) : 'Мұғалім бағасы мен пікірі осы жерде көрсетіледі.'}</p><div class="progress-bar"><span style="width:${answered / 7 * 100}%"></span></div></article><article class="metric-card"><small>Орындалған тапсырмалар</small><strong>${taskDone}/${assignments.length}</strong><p>${methodDone === 5 ? 'CODE THINK кезеңдері толық меңгерілді.' : `CODE THINK: ${methodDone}/5 кезең`}</p><div class="progress-bar"><span style="width:${assignments.length ? taskDone / assignments.length * 100 : 0}%"></span></div></article></div><div class="dashboard-section"><div class="section-mini-heading"><div><span class="kicker">ЖЕТІСТІКТЕР</span><h2>Менің белгілерім</h2></div><p>${achievements.filter(x=>x.earned).length}/${achievements.length} белгі ашылды</p></div><div class="achievement-grid">${achievements.map(x=>`<article class="achievement ${x.earned?'earned':''}"><b>${x.icon}</b><strong>${escapeHtml(x.title)}</strong><small>${escapeHtml(x.text)}</small></article>`).join('')}</div></div><div class="dashboard-section"><div class="section-mini-heading"><div><span class="kicker">7 ДАҒДЫ</span><h2>Алгоритмдік ойлау профилі</h2></div><p>${finalAttempt?.status === 'graded' ? 'Бастапқы және қорытынды нәтиже салыстырылды.' : 'Қорытынды диагностикадан кейін өсім көрсетіледі.'}</p></div>${comparison}</div><div class="student-dashboard-grid"><article class="dashboard-section"><span class="kicker">КЕЛЕСІ ҚАДАМ</span><h2>Не істеу керек?</h2><p class="recommendation">${escapeHtml(nextAction)}</p></article><article class="dashboard-section"><span class="kicker">ТАПСЫРМАЛАР</span><h2>Оқу жұмыстары</h2><ul class="compact-list">${assignmentItems}</ul></article></div>`;
}
...
function renderTeacherData() {
  ...
  populateTeacherFilters(); renderDiagnosticSubmissions(); renderStudentList(); renderTeacherAnalytics();
}
function populateSelect(select, values, placeholder) { if (!select) return; const current=select.value; select.innerHTML=`<option value="">${placeholder}</option>`+values.map(v=>`<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join(''); select.value=current; }
function populateTeacherFilters() {
  const classes=[...new Set((state.teacherData.users||[]).map(u=>u.className||`${u.grade}-сынып`).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'kk'));
  populateSelect($('#diagnosticClassFilter'),classes,'Барлық сынып'); populateSelect($('#taskClassFilter'),classes,'Барлық сынып'); populateSelect($('#studentClassFilter'),classes,'Барлық сынып');
  if ($('#bulkStudentClass')) { const current=$('#bulkStudentClass').value; $('#bulkStudentClass').innerHTML='<option value="">Сыныпқа ауыстыру...</option>'+(state.teacherWorkspace.classes||[]).map(c=>`<option value="${c.id}">${escapeHtml(c.name)}</option>`).join(''); $('#bulkStudentClass').value=current; }
}
function renderDiagnosticSubmissions() {
  const search=normalize_($('#diagnosticSearch')?.value), className=$('#diagnosticClassFilter')?.value||'', status=$('#diagnosticStatusFilter')?.value||'', from=$('#diagnosticDateFrom')?.value, to=$('#diagnosticDateTo')?.value;
  const submitted=(state.teacherData.attempts||[]).filter(a=>a.status!=='in_progress').filter(r=>{ const date=String(r.submittedAt||r.startedAt||'').slice(0,10); return (!search||normalize_(`${r.student.name} ${r.student.login}`).includes(search))&&(!className||(r.student.className||`${r.student.grade}-сынып`)===className)&&(!status||r.status===status)&&(!from||date>=from)&&(!to||date<=to); });
  $('#submissionList').innerHTML = submitted.length ? submitted.map(record => {
    ...
    return `<details class="submission" data-record="${record.id}"><summary><input class="submission-select" type="checkbox"><span class="submission-name"><strong>${escapeHtml(record.student.name)} · ${typeLabel}</strong><small>${escapeHtml(record.student.className || record.student.grade + '-сынып')} · ${formatDate(record.submittedAt)} · беттен ауысу: ${record.switches || 0}</small></span><span class="status-pill">${record.status === 'graded' ? 'Бағаланды' : 'Тексеру керек'}</span><span class="score-total">${total !== null ? total + '/21' : '— /21'}</span></summary><div class="review-body"><div class="quick-grade"><span>Жылдам бағалау:</span>${[1,2,3].map(n=>`<button class="small-button quick-score" type="button" data-value="${n}">Барлығына ${n}</button>`).join('')}<button class="small-button feedback-preset" type="button" data-text="Жауап жақсы негізделген. Келесі жолы шешімді мысалмен толықтыр.">Дайын пікір</button></div>${diagnosticTasks(record.grade, attemptType(record)).map((task, i) => { const answer = (record.answers || []).find(a => Number(a.taskIndex) === i) || {}; return `<div class="review-item"><div><h4>${i + 1}. ${task.title}</h4><div class="answer-text">${escapeHtml(answer.answer || 'Жауап берілмеген')}</div><textarea class="feedback-input" data-feedback="${i}" placeholder="Оқушыға пікір...">${escapeHtml(answer.feedback || '')}</textarea></div><select class="score-select" data-score="${i}">${[0,1,2,3].map(n=>`<option value="${n}" ${Number(answer.score)===n&&answer.score!==''?'selected':''}>${n} балл</option>`).join('')}</select></div>`; }).join('')}<label class="field-label teacher-comment">Жалпы пікір<textarea data-comment>${escapeHtml(record.teacherComment||'')}</textarea></label><div class="review-actions"><button class="primary save-scores">Бағаны сақтау</button></div></div></details>`;
  }).join('') : '<div class="empty-state"><h2>Жұмыс табылмады</h2><p>Сүзгіні өзгертіңіз немесе оқушы жұмысын күтіңіз.</p></div>';
  $$('.save-scores').forEach(btn=>btn.addEventListener('click',()=>saveScores(btn.closest('[data-record]'),btn)));
  $$('.quick-score').forEach(btn=>btn.addEventListener('click',()=>$$('.score-select',btn.closest('[data-record]')).forEach(x=>x.value=btn.dataset.value)));
  $$('.feedback-preset').forEach(btn=>btn.addEventListener('click',()=>{ $('[data-comment]',btn.closest('[data-record]')).value=btn.dataset.text; }));
  $$('.submission-select').forEach(x=>x.addEventListener('click',e=>e.stopPropagation()));
}
function renderStudentList() {
  const search=normalize_($('#studentSearch')?.value), className=$('#studentClassFilter')?.value||'', status=$('#studentStateFilter')?.value||'';
  const users=(state.teacherData.users||[]).filter(u=>(!search||normalize_(`${u.name} ${u.login}`).includes(search))&&(!className||(u.className||`${u.grade}-сынып`)===className)&&(!status||(status==='active'?u.active:!u.active)));
  $('#studentList').innerHTML = users.length ? users.map(u => `<div class="student-row" data-user="${u.id}"><div><label><input class="student-select" type="checkbox" value="${u.id}"><strong>${escapeHtml(u.name)}</strong></label><small>${escapeHtml(u.login)} · ${escapeHtml(u.className || u.grade + '-сынып')} · ${u.active ? 'белсенді' : 'бұғатталған'} · соңғы кіру: ${formatDate(u.lastLoginAt)}</small></div><div class="student-actions"><button class="small-button show-student-progress">Прогресс</button><button class="small-button reset-password">Жаңа пароль</button><button class="small-button toggle-user">${u.active ? 'Бұғаттау' : 'Ашу'}</button></div><div class="student-snapshot hidden">${studentSnapshotHtml(u)}</div></div>`).join('') : '<p class="helper">Сүзгіге сәйкес оқушы табылмады.</p>';
...
function renderTeacherAnalytics() {
  ...
  const taskAttempts=state.teacherData.taskAttempts||[], neverLogged=users.filter(u=>!u.lastLoginAt).length, started=users.filter(u=>attempts.some(a=>a.userId===u.id)||taskAttempts.some(a=>a.userId===u.id)).length, completed=users.filter(u=>attempts.some(a=>a.userId===u.id&&a.status==='graded')||taskAttempts.some(a=>a.userId===u.id&&a.status==='graded')).length;
...
function renderTaskSubmissions() {
  ...
  const search=normalize_($('#taskSearch')?.value), className=$('#taskClassFilter')?.value||'', status=$('#taskStatusFilter')?.value||'', from=$('#taskDateFrom')?.value, to=$('#taskDateTo')?.value;
  const attempts = assignments.flatMap(a => (a.attempts || []).filter(x => x.status === 'submitted' || x.status === 'graded').map(x => ({ ...x, assignment: a }))).filter(item=>{ const date=String(item.submittedAt||'').slice(0,10); return (!search||normalize_(`${item.student.name} ${item.assignment.title}`).includes(search))&&(!className||(item.assignment.classInfo?.name||'')===className)&&(!status||item.status===status)&&(!from||date>=from)&&(!to||date<=to); });
  $('#taskSubmissionList').innerHTML = attempts.length ? attempts.map(item => `<details class="submission" data-task-attempt="${item.id}"><summary><span class="submission-name"><strong>${escapeHtml(item.student.name)}</strong><small>${escapeHtml(item.assignment.title)} · ${escapeHtml(item.assignment.classInfo?.name || '')}</small></span><span class="status-pill">${item.status === 'graded' ? 'Бағаланды' : 'Тексеру керек'}</span><span class="score-total">${item.status === 'graded' ? item.score + '/15' : '— /15'}</span></summary><div class="review-body"><div class="quick-grade"><span>Жылдам бағалау:</span>${[1,2,3].map(n=>`<button class="small-button quick-task-score" type="button" data-value="${n}">Барлығына ${n}</button>`).join('')}<button class="small-button task-feedback-preset" type="button" data-text="Алгоритм дұрыс құрылған. Түсіндіруді нақты мысалмен толықтыр.">Дайын пікір</button></div>${methodSteps.map(step => { const code = step.code, data = (item.steps || []).find(s => s.stepCode === code) || {}; return `<div class="review-item"><div><h4>${escapeHtml(step.title)} · ${escapeHtml(step.english)}</h4><div class="answer-text">${escapeHtml(data.response || 'Жауап берілмеген')}</div><textarea class="feedback-input" data-task-feedback="${code}" placeholder="Пікір...">${escapeHtml(data.feedback || '')}</textarea></div><select class="score-select" data-task-score="${code}">${[0,1,2,3].map(n => `<option value="${n}" ${Number(data.score) === n && data.score !== '' ? 'selected' : ''}>${n} балл</option>`).join('')}</select></div>`; }).join('')}<div class="review-item"><div><h4>Python коды</h4><pre class="code-block">${escapeHtml(item.code || 'Код жазылмаған')}</pre><div class="answer-text">Нәтиже: ${escapeHtml(item.output || '—')}</div></div></div><label class="field-label teacher-comment">Жалпы пікір<textarea data-task-comment>${escapeHtml(item.teacherComment || '')}</textarea></label><div class="review-actions"><button class="primary save-task-grade">Бағаны сақтау</button></div></div></details>`).join('') : '<div class="empty-state"><h2>Тапсырылған жұмыс табылмады</h2><p>Сүзгіні өзгертіңіз немесе жаңа жұмысты күтіңіз.</p></div>';
  $$('.save-task-grade').forEach(btn => btn.addEventListener('click', () => gradeTaskAttempt(btn.closest('[data-task-attempt]'), btn)));
  $$('.quick-task-score').forEach(btn=>btn.addEventListener('click',()=>$$('[data-task-score]',btn.closest('[data-task-attempt]')).forEach(x=>x.value=btn.dataset.value)));
  $$('.task-feedback-preset').forEach(btn=>btn.addEventListener('click',()=>{ $('[data-task-comment]',btn.closest('[data-task-attempt]')).value=btn.dataset.text; }));
}
...
function selectedStudentIds() { return $$('.student-select:checked').map(x=>x.value); }
async function bulkStudentOperation(operation) {
  const userIds=selectedStudentIds(); if (!userIds.length) return toast('Алдымен оқушыларды таңдаңыз',true);
  const classId=operation==='move'?$('#bulkStudentClass').value:''; if (operation==='move'&&!classId) return toast('Ауыстыратын сыныпты таңдаңыз',true);
  const labels={activate:'аккаунттарын ашу',block:'аккаунттарын бұғаттау',move:'басқа сыныпқа ауыстыру',reset:'парольдерін жаңарту'};
  if (!confirm(`${userIds.length} оқушының ${labels[operation]} әрекетін орындайсыз ба?`)) return;
  try { const result=await api('bulkManageStudents',{userIds,operation,classId}); if (result.credentials?.length) downloadBulkCredentials(result.credentials); await syncTeacherData(); await refreshTeacherWorkspace(); toast(`${result.updated} оқушы жаңартылды`); } catch(err){ toast(err.message,true); }
}
function downloadBulkCredentials(rows) {
  requireXlsx(); const data=rows.map(x=>({'Аты-жөні':x.name,'Логин':x.login,'Сынып':x.className,'Уақытша құпиясөз':x.temporaryPassword,'Нәтиже':x.status}));
  const sheet=XLSX.utils.json_to_sheet(data); sheet['!cols']=[{wch:28},{wch:18},{wch:16},{wch:24},{wch:28}]; const book=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(book,sheet,'Жаңа парольдер'); XLSX.writeFile(book,'CODE_THINK_жаңа_парольдер.xlsx');
}
async function applyBulkDiagnosticComment() {
  const attempts=$$('.submission-select:checked').map(x=>x.closest('[data-record]').dataset.record), comment=$('#bulkDiagnosticComment').value.trim(), score=$('#bulkDiagnosticScore').value;
  if (!attempts.length) return toast('Жұмыстарды белгілеңіз',true); if (!comment&&!score) return toast('Ортақ пікірді немесе баллды таңдаңыз',true);
  if (score&&!confirm(`${attempts.length} жұмыстың барлық 7 тапсырмасына ${score} балл қойылады. Жалғастырасыз ба?`)) return;
  try { const result=score?await api('bulkGradeAttempts',{attemptIds:attempts,score:Number(score),comment}):await api('bulkCommentAttempts',{attemptIds:attempts,comment}); await syncTeacherData(); toast(`${result.updated} жұмыс жаңартылды`); } catch(err){ toast(err.message,true); }
}
...
['diagnosticSearch','diagnosticClassFilter','diagnosticStatusFilter','diagnosticDateFrom','diagnosticDateTo'].forEach(id=>$('#'+id).addEventListener(id==='diagnosticSearch'?'input':'change',renderDiagnosticSubmissions));
['taskSearch','taskClassFilter','taskStatusFilter','taskDateFrom','taskDateTo'].forEach(id=>$('#'+id).addEventListener(id==='taskSearch'?'input':'change',renderTaskSubmissions));
['studentSearch','studentClassFilter','studentStateFilter'].forEach(id=>$('#'+id).addEventListener(id==='studentSearch'?'input':'change',renderStudentList));
$('#selectAllStudents').addEventListener('change',event=>$$('.student-select').forEach(x=>x.checked=event.target.checked));
$('#bulkMoveStudents').addEventListener('click',()=>bulkStudentOperation('move')); $('#bulkActivateStudents').addEventListener('click',()=>bulkStudentOperation('activate')); $('#bulkBlockStudents').addEventListener('click',()=>bulkStudentOperation('block')); $('#bulkResetPasswords').addEventListener('click',()=>bulkStudentOperation('reset'));
$('#applyBulkDiagnosticComment').addEventListener('click',applyBulkDiagnosticComment);
